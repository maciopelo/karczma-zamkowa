...
export default function Alternative() {
  return (
    <>
      <section
        className="flex min-h-screen w-full items-center justify-center bg-neutral-600/85 bg-cover bg-center px-4 bg-blend-multiply"
        style={{ backgroundImage: "url('/hero.webp')" }}
      >
        <div className="flex h-full w-full flex-col items-center justify-center gap-10 pt-30 pb-10 text-4xl text-white sm:gap-20 md:py-40">
          <h1
            className={cn(
          ...
          <nav className="flex flex-wrap items-center justify-center gap-5 sm:gap-10 xl:gap-20">
            {nav.map((item) => (
              <Link
                key={item.label}
                href={item.link}
                className="relative flex h-40 w-50 items-center justify-center rounded-2xl border border-stone-200 bg-neutral-950/85 md:h-80 md:w-xs"
              >
